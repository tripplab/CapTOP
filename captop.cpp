...
static void print_persist_options(std::ostream& os) {
    ...
    os << "  --write-grid             Also dump captop_occupied_u8.raw, captop_cube_values_f64.raw\n";
    os << "                           (the exact field GUDHI saw), and captop_grid_metadata.json into\n";
    os << "                           the output dir, so a downstream consumer can reconstruct the grid\n";
    os << "                           and compute a perturbation noise floor without re-running captop\n";
...
struct PersistOptions {
  ...
  bool overwrite=false, force=false, quiet=false, write_pairs=false, write_diagrams=false, write_betti_curve=false, write_barcode_summary=false, write_json=false, write_all=false, finite_only=false, include_essential=true, write_grid=false;
  SdtOptions sdt;            // Stage 7: signed distance transform filtration
};
...
static int run_persist(int argc,char** argv){
  ...
  for(int i=3;i<argc;++i){ std::string a=argv[i]; auto need=[&](const std::string& n){ if(i+1>=argc) throw std::runtime_error(n+" requires a value"); return std::string(argv[++i]);};
    try{ if(a=="--grid"){ auto v=to_lower(need(a)); if(v=="strict"||v=="strict-cube") vo.grid_mode=GridMode::StrictCube; else if(v=="rectilinear") vo.grid_mode=GridMode::Rectilinear; else throw std::runtime_error("unsupported grid mode"); }
      ...
      else if(a=="--write-pairs") opt.write_pairs=true; else if(a=="--write-diagrams") opt.write_diagrams=true; else if(a=="--write-betti-curve") opt.write_betti_curve=true; else if(a=="--write-barcode-summary") opt.write_barcode_summary=true; else if(a=="--write-json") opt.write_json=true; else if(a=="--write-all") opt.write_all=true; else if(a=="--write-grid") opt.write_grid=true; else if(a=="--finite-only"){ opt.finite_only=true; opt.include_essential=false; } else if(a=="--include-essential") opt.include_essential=true; else if(a=="--essential-death") (void)need(a);
  ...
  try{
    ...
    /* Stage 7.1: --write-grid. Dump the dense occupancy bitmap and the actual
       filtration value field (phys) that GUDHI was given, plus a grid-metadata
       sidecar identical in schema to convert's. This lets the Python metric layer
       (cubic_fold_compare.py) reconstruct the exact grid CapTOP used and: (a)
       reproduce this diagram from the field as a self-check before trusting any
       perturbed diagram, and (b) compute a +/-1-voxel selector-stability noise floor
       by morphologically nudging the occupancy and recomputing, WITHOUT re-running
       CapTOP. Occupancy is defined from the validated cell list (correct for any
       filtration, including signed SDT where empty cells also carry finite values).
       Files are co-located with the diagrams so one fold lives in one directory. */
    bool grid_written = false;
    if(opt.write_grid){
        std::vector<uint8_t> occ_dump(total, 0);
        for(const auto& ic: vr.indexed_cells) occ_dump[linear_index(ic.i,ic.j,ic.k,vr.nx,vr.ny)] = 1;
        write_raw(od/"captop_occupied_u8.raw", occ_dump);
        write_raw(od/"captop_cube_values_f64.raw", phys);   // physical field GUDHI saw
        { std::ofstream js(od/"captop_grid_metadata.json"); js<<std::setprecision(17)
            <<"{\n  \"software\": {\"name\": \"captop\", \"version\": \""<<CAPTOP_VERSION<<"\"},\n"
            <<"  \"origin_command\": \"persist --write-grid\",\n"
            <<"  \"input\": {\"path\": \""<<json_escape(input)<<"\"},\n"
            <<"  \"grid\": {\"nx\": "<<vr.nx<<", \"ny\": "<<vr.ny<<", \"nz\": "<<vr.nz
            <<", \"total_cells\": "<<total<<", \"origin\": ["<<vr.origin.x<<", "<<vr.origin.y<<", "<<vr.origin.z
            <<"], \"spacing\": ["<<vr.spacing.x<<", "<<vr.spacing.y<<", "<<vr.spacing.z
          ...
          js<<"},\n  \"raw_files\": {\"occupied_u8\": {\"path\": \"captop_occupied_u8.raw\", \"type\": \"uint8\", \"count\": "<<total
            ...
            <<", \"missing_value\": \"+inf\", \"note\": \"physical filtration field passed to GUDHI (birth_physical/death_physical axis)\"}}\n}\n";
          if(!js) throw std::runtime_error("cannot write captop_grid_metadata.json"); }
        grid_written = true;
    }
    ...
    { std::ofstream f(od/"captop_diagram_manifest.json"); f<<std::setprecision(17);
      ...
      // Stage 7.1: advertise co-located grid raw files when --write-grid was used.
      // A Python consumer reads this to decide whether it can run the in-process
      // +/-1-voxel selector-stability noise floor (needs occupancy + field) or must
      // skip it. null when --write-grid was not passed.
      if(grid_written){
        f<<"  \"grid_files\": {\"present\": true, \"dir\": \".\", "
         <<"\"grid_metadata\": \"captop_grid_metadata.json\", "
         <<"\"occupied_u8\": {\"path\": \"captop_occupied_u8.raw\", \"type\": \"uint8\", \"count\": "<<total<<"}, "
         <<"\"cube_values_f64\": {\"path\": \"captop_cube_values_f64.raw\", \"type\": \"float64\", \"endianness\": \"little\", \"count\": "<<total
         <<", \"is_filtration_field\": true, \"note\": \"exact physical field passed to GUDHI; feed to a Python cubical complex to reproduce these diagrams before perturbing\"}, "
         <<"\"index_order\": \"i + nx * (j + ny * k)\", \"axis_order\": [\"x\", \"y\", \"z\"], "
         <<"\"grid\": {\"nx\": "<<vr.nx<<", \"ny\": "<<vr.ny<<", \"nz\": "<<vr.nz<<", \"spacing\": ["<<vr.spacing.x<<", "<<vr.spacing.y<<", "<<vr.spacing.z<<"], \"origin\": ["<<vr.origin.x<<", "<<vr.origin.y<<", "<<vr.origin.z<<"]}},\n";
      } else {
        f<<"  \"grid_files\": null,\n";
      }
